...
> 后续模型复核：用户已解除 §6.3 止损。独立重算含截距模型得到 handoff
> U=20.4139µs、F=6.9046ms，default U=19.5626µs、F=441.3741ms；原来的
> residual/N 比值不能否定固定边际成本。两点拟合仍不证明因果 F，尤其 handoff
> 逻辑工作并非不变。下文保留当时决策及全部原始负结果；当前工作改为 E1-COLD
> 归因和 T1-PROD 契约，不再以此 §6.3 拒绝 grant 机制。见
> [后续证据](../e1-cold-t1-prod/README.md)。
