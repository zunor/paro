...
from benchmark.harness.result_protocol import (
    normalize_value_v1,
    ordered_rows_digest_v1,
)
...
class ResultNormalizationTests(unittest.TestCase):
    def test_decimal_normalization_is_exact_and_preserves_scale(self) -> None:
        self.assertEqual(normalize_value_v1(Decimal("123141078.2283")), "123141078.2283")
        self.assertEqual(normalize_value_v1(Decimal("37734107.00")), "37734107.00")

    def test_ordered_digest_preserves_order_and_value_boundaries(self) -> None:
        rows = [["ab", "c"], [1, None]]
        self.assertEqual(
            ordered_rows_digest_v1(rows),
            "5140ee4de11a3233a4d8de72ad3cc382470b849eb0e4f88d091ffc428c587de0",
        )
        self.assertNotEqual(
            ordered_rows_digest_v1(rows), ordered_rows_digest_v1(list(reversed(rows)))
        )
        self.assertIn("ordered_digest_v1", STRONG_VALIDATE_MODES)
