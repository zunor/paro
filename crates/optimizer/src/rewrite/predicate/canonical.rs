...
pub(crate) fn finish(plan: OwnedLogicalPlan) -> Result<OwnedLogicalPlan> {
    ...
    plan.try_map_post_order(|plan| {
        Ok(crate::rewrite::predicate::restriction::normalize_node(
...
mod tests {
    ...
    use paro_planner::{
        ...
        logical::operator::{EmptyResult, Filter, LogicalOperator},
    };
    ...
    fn predicate_construction_matches_separate_routing_oracle() {
        ...
        use paro_planner::logical::operator::{ColumnBinding, ExpressionGet, Limit, Projection};
            ...
            for nesting in 0..12 {
                ...
                for plan in [&mut actual, &mut expected] {
                    plan.visit_post_order_mut(|node| {
                        node.id = paro_planner::logical::plan::PlanNodeId(0)
                    });
                }
                assert_eq!(
                    format!("{actual:?}"),
    ...
    fn local_construction_matches_separate_pass_oracle() {
        for empty_input in [false, true] {
            for depth in 0..12 {
                ...
                copied.visit_post_order_mut(|node| {
                    node.id = paro_planner::logical::plan::PlanNodeId(0)
                });
                let old = crate::rewrite::subquery::empty_result::EmptyResultPullup::new()
                    .optimize_plan(copied);
                let expected = crate::rewrite::predicate::restriction::normalize_tree(old).unwrap();
