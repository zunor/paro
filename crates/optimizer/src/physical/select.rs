...
struct SourceResponse {
    source: crate::cost::response::WorkSourceId,
    base: SearchCost,
    charged: SearchCost,
    retained_ppm: u32,
}
...
pub(crate) fn estimate_response(
    ...
    let _scope = crate::diagnostics::work::enter(crate::diagnostics::work::Bucket::Kernel);
    Ok(
...
mod tests {
    ...
    fn rf_facts() -> ResolvedPlannerCostFacts {
        ...
        facts.runtime_filter_probe_sources = Box::new([ResolvedRuntimeFilterSource {
            source: crate::cost::response::WorkSourceId(7),
    ...
    fn rf_response_is_source_scoped_and_repeated_demands_are_idempotent() {
        ...
        facts.runtime_filter_probe_sources[0].source = crate::cost::response::WorkSourceId(8);
    ...
    fn child(cost: SearchCost) -> PhysicalResponse {
        PhysicalResponse {
            cost,
            hard_rows: None,
