...
//! Eligibility and costing do not construct alternative plans.
...
/// Resolve a selected tree's local facts from its committed inputs.
...
pub(crate) use runtime_filter::{
    planner_source_lineage, supports_build_left_runtime_filter_auxiliary,
    supports_runtime_filter_auxiliary, RuntimeFilterInput,
};
...
/// recreating `SubplanRef` plans here would only pay owned-IR allocation
