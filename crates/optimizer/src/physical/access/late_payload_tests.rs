...
fn rejection_guards_observe_actual_late_payload_proofs_without_changing_results() {
    ...
    for (plan, expected) in cases {
        ...
        let (normal, normal_changed) = super::late_payload::rewrite_node(
            observed,
            &BindContext::new(),
            &SelectivityModel::default(),
        )
...
fn candidate(order_by_payload: bool) -> OwnedLogicalPlan {
...
fn candidate_with_null_extended_source() -> OwnedLogicalPlan {
...
fn selective_projection_candidate(source: GetColumnSource) -> OwnedLogicalPlan {
    let table = source_table();
    let mut get = Get::new(
        SOURCE,
        table
...
fn selective_join_projection_candidate(
    ...
    let mut join = OwnedLogicalPlan::synthetic(LogicalOperator::Join(Join::Comparison(
        ComparisonJoin::new(
            join_type,
            left,
            right,
...
fn row_preserving_candidate(
    ...
    OwnedLogicalPlan::synthetic(LogicalOperator::TopN(topn))
}
...
fn bounded_topn_replaces_wide_dependent_groups_with_rowid() {
    ...
    let (optimized, changed) =
        optimize_plan(candidate(false), &context, &SelectivityModel::default()).unwrap();
    assert!(changed);
...
fn ordering_by_delayed_payload_keeps_preserving_plan() {
    let context = BindContext::new();
    let (optimized, changed) =
        optimize_plan(candidate(true), &context, &SelectivityModel::default()).unwrap();
    assert!(!changed);
...
fn three_matching_gets_never_restore_false_uniqueness() {
    ...
    let three = OwnedLogicalPlan::synthetic(LogicalOperator::Join(Join::Cross(CrossProduct::new(
        two,
        source(),
    ))));
...
fn verifier_rejects_projection_name_and_get_physical_layout_drift() {
    ...
    let plan = OwnedLogicalPlan::synthetic(LogicalOperator::Projection(projection));
    let error = crate::verify::verify_logical_plan(&context, &plan)
