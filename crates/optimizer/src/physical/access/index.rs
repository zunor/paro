...
use paro_planner::logical::plan::{LogicalInput, LogicalPlanRead};
...
use crate::cost::access::{FullTextScanCostModel, VectorScanCostModel};
...
impl AccessPlanner {
    ...
    fn try_rewrite_fulltext_filter<C>(
        ...
        for (match_idx, expr) in filter.expressions.iter().enumerate() {
            ...
            let (_, residual) =
                crate::physical::lower::predicate_builder::build_search_predicate_template(
                    &required_filters,
                    get,
                )?;
...
fn build_search_scan<C>(
    ...
    let (_, residual) = crate::physical::lower::predicate_builder::build_search_predicate_template(
        &candidate_filters,
        pattern.get,
    )?;
    if !residual.is_empty() {
...
fn extract_topn_pattern<C: paro_planner::logical::plan::LogicalChild>(
    topn: &TopN<C>,
