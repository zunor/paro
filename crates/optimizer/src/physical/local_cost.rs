...
use super::{Fingerprint, PhysicalImplementationFlavor};
...
pub(crate) fn fit_local_cost(
    mut cost: SearchCost,
    spillable: bool,
    class: super::ResourceGrantClass,
    force_spill: bool,
) -> Result<Option<SearchCost>> {
    if cost.minimum_memory_bytes > class.hard_memory_bytes {
        return Ok(None);
    }
    if spillable && class.spill_policy == super::SpillPolicy::Forbidden {
    ...
    if cost.peak_memory_upper == u64::MAX {
        if spillable && class.spill_policy == super::SpillPolicy::Allowed {
    ...
    if force_spill && spillable {
        if class.spill_policy == super::SpillPolicy::Forbidden {
            return Ok(None);
        } else {
...
pub(crate) struct LocalCostModel<'a> {
    ...
    pub spillable: bool,
    pub perfect_hash: Option<super::PerfectHashResourceContract>,
...
pub(crate) struct ResolvedPlannerCostFacts {
    ...
    pub(crate) runtime_filter_probe_source_rows: Option<CompactRange>,
    pub(crate) runtime_filter_build_left_probe_source_rows: Option<CompactRange>,
...
pub(crate) enum RuntimeFilterProbeMultiplicity {
    #[default]
    Unknown,
...
pub(crate) fn runtime_filtered_probe_work(
    probe: CompactRange,
    build: CompactRange,
