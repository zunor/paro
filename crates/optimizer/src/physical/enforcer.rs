...
use super::identity::{EnforcerRecipeId, Fingerprint, MutationBarrierId, StableFingerprintBuilder};
...
pub enum EnforcerStep {
    Gather,
    RepartitionHash {
        keys: Box<[super::identity::ColumnId]>,
        partitions: u16,
    },
    RepartitionRange {
        keys: Box<[super::identity::ColumnId]>,
    ...
    Fetch {
        values: BTreeSet<super::identity::ColumnId>,
    },
    ...
    Flatten,
    Factorize(super::identity::FactorizationSpecId),
    Spool,
}
...
fn encode_columns<'a>(
    ...
    columns: impl IntoIterator<Item = &'a super::identity::ColumnId>,
) {
...
fn encode_ordering(builder: &mut StableFingerprintBuilder, ordering: &RequiredOrdering) {
    ...
    for key in &ordering.keys {
        ...
        builder.write_u64(match key.direction {
            super::requirements::SortDirection::Asc => 0,
            super::requirements::SortDirection::Desc => 1,
        });
        builder.write_u64(match key.nulls {
            super::requirements::NullOrder::First => 0,
            super::requirements::NullOrder::Last => 1,
        });
        match key.collation {
            Some(collation) => {
                builder.write_u64(1);
