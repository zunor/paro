...
impl PhysicalPlanExtractor {
    pub(crate) fn lower_get(
        ...
        if get.table.is_none() {
            return Err(paro_error::internal(
                "base table GET reached physical extraction without catalog metadata",
            ));
        }
    ...
    pub(crate) fn lower_filter(
        ...
        // boundary that the executable ABI does not need.
        ...
        let child = self.extract_node(filter.child.as_ref())?;
    ...
    fn lower_aggregate_filter(
        ...
        let (aggregate_kind, aggregate_children) =
            self.lower_aggregate_with_having(aggregate, having_filter, implementation)?;
    ...
    pub(crate) fn lower_limit(
        ...
        Ok((PhysicalNodeKind::Limit(Box::new(spec)), vec![child]))
    }
    ...
    pub(crate) fn lower_search_scan(
        ...
        if !residual.is_empty() {
            return Err(paro_error::internal(
                "search provider candidate retained a residual predicate without a physical filter",
            ));
        }
...
fn search_source_spec_for_candidate(
    ...
    // filters are represented by `predicate`; candidates with a residual are
    // rejected before physical extraction. Keep that proof explicit and
    // separately describe how its exact row set is obtained.
