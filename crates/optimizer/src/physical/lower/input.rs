...
pub(crate) type PreparedChild = Arc<PreparedNode>;

#[derive(Debug)]
pub(crate) struct PreparedNode {
    pub id: PlanNodeId,
    pub stats: NodeStats,
    pub operator: LogicalOperator<PreparedChild>,
...
impl PreparedNode {
    ...
    #[cfg(test)]
    pub fn children(&self) -> Vec<&PreparedNode> {
    ...
    pub fn from_local(
        ...
        paro_planner::logical::verify::verify_physical_planner_invariants(&local.operator)?;
        if replay_keys {
        ...
        {
            crate::physical::slot_assignment::assign_expression_slots(&mut local.operator)?;
        }
        let mut children = children.into_iter();
        let (id, stats, operator) = local.into_parts();
        let operator = operator.try_map_child_links(&mut |boundary| -> Result<PreparedChild> {
            let child = children
                .next()
    ...
    pub fn from_owned(plan: OwnedLogicalPlan) -> Result<PreparedChild> {
        ...
        let mut completed: Vec<PreparedChild> = Vec::new();
...
impl paro_planner::logical::plan::LogicalInput for PreparedNode {
...
impl paro_planner::logical::plan::LogicalPlanRead for PreparedNode {
    type Child = PreparedChild;
    fn operator(&self) -> &LogicalOperator<Self::Child> {
        &self.operator
    }
}

impl Drop for PreparedNode {
    fn drop(&mut self) {
        fn detach(node: &mut PreparedNode, pending: &mut Vec<PreparedChild>) {
...
mod tests {
    ...
    use paro_planner::logical::operator::{Filter, Projection};
    ...
    fn selected_inputs_keep_bindings_while_scalars_become_slots() {
        ...
        let selected = PreparedNode::from_owned(parent).unwrap();
    ...
    fn selected_shell_rejects_a_mismatched_child_contract() {
        ...
        assert!(PreparedNode::from_local(local, vec![], true).is_err());
    }
    ...
    fn deep_selected_inputs_construct_and_drop_without_recursive_transport() {
        std::thread::Builder::new()
            ...
            .spawn(|| {
                ...
                let selected = PreparedNode::from_owned(plan).unwrap();
                let mut count = 0;
                selected
