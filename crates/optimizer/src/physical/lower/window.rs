...
impl PhysicalPlanBuilder {
    pub(crate) fn lower_window(
        &mut self,
        window: &LogicalWindow<PreparedChild>,
        ...
        let mut child = self.extract_node(window.child.as_ref())?;
        ...
        let partition_aggregate = match implementation {
            crate::physical::PhysicalImplementationFlavor::PartitionAggregateWindow
            | crate::physical::PhysicalImplementationFlavor::Structural => {
                lower_partition_aggregate_window_spec(window, output_names.clone())?
            }
        ...
        if let Some(spec) = partition_aggregate {
            return Ok((
                PhysicalNodeKind::PartitionAggregateWindow(Box::new(spec)),
                vec![child],
            ));
        }
...
pub(crate) fn supports_partition_aggregate_window<
    Child: paro_planner::logical::plan::LogicalInput,
>(
    ...
    let output_names = (0..window.get_types().len())
        ...
        .collect();
    matches!(
        lower_partition_aggregate_window_spec(window, output_names),
        Ok(Some(_))
    )
}
...
fn lower_partition_aggregate_window_spec<Child: paro_planner::logical::plan::LogicalInput>(
    window: &LogicalWindow<Child>,
