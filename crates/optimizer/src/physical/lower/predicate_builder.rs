...
use paro_function::scalar::cast::CastContextDependency;
...
fn extract_comparison_constant(
    ...
    match operand.transform {
        ...
        ScanColumnTransform::IntegralWidening => {
            let Some(value) = evaluate_constant_expression(expr)? else {
                return Ok(None);
            };
        ...
        ScanColumnTransform::DateToTimestamp => {
            ...
            let Some(Value::Timestamp(timestamp)) = evaluate_constant_expression(expr)? else {
                return Ok(None);
            };
...
fn build_operator_predicate(
    ...
    match op.operator_type {
        ...
        OperatorType::IsNull | OperatorType::IsNotNull => {
            let child = match op.children.first() {
                Some(child) => child,
                None => return Ok(None),
            };
...
fn build_projected_prefix_membership(
    ...
    for child in &op.children[1..] {
        let Some(Value::Varchar(value)) = evaluate_constant_expression(child)? else {
            return Ok(None);
        };
...
fn build_like_predicate(
    ...
    let Some(Value::Varchar(pattern)) = evaluate_constant_expression(pattern)? else {
        return Ok(None);
    };
    let Some(column_id) = get.stored_column(col_idx) else {
        return Ok(None);
