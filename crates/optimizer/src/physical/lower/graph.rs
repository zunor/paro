...
impl PhysicalPlanBuilder {
    pub(crate) fn lower_graph_scan(
        ...
        (PhysicalNodeKind::GraphScan(Box::new(spec)), Vec::new())
    }
    ...
    pub(crate) fn lower_graph_expand(
        ...
        let child = self.extract_node(expand.child.as_ref())?;
        ...
        if matches!(
            ...
            return Ok((
                PhysicalNodeKind::GraphShortestPath(Box::new(spec)),
                vec![child],
            ));
        }
        ...
        Ok((PhysicalNodeKind::GraphExpand(Box::new(spec)), vec![child]))
    }
