...
fn unique_build_proof_resolves_physical_references_through_carriers() {
    ...
    let projection =
        crate::estimate::unique_keys::refresh_unique_keys(projection).expect("cache unique keys");
...
fn unique_build_proof_propagates_through_windows() {
    ...
    let window =
        crate::estimate::unique_keys::refresh_unique_keys(window).expect("cache unique keys");
    let conditions = [JoinCondition::new(
...
fn graph_expand_does_not_promote_its_input_key_to_an_output_key() {
    ...
    let expanded =
        crate::estimate::unique_keys::refresh_unique_keys(expanded).expect("cache unique keys");
    let conditions = [JoinCondition::new(
        Expression::Reference(ReferenceExpression::new(0, LogicalType::Varchar).into()),
        Expression::Reference(ReferenceExpression::new(1, LogicalType::BigInt).into()),
        JoinComparisonType::Equal,
...
fn unique_build_proof_requires_a_key_preserving_join() {
    ...
    let multiplicative = crate::estimate::unique_keys::refresh_unique_keys(multiplicative)
    ...
    let preserving =
        crate::estimate::unique_keys::refresh_unique_keys(preserving).expect("cache unique keys");
    assert!(crate::estimate::unique_keys::expressions_cover_unique_key(
        &preserving,
...
fn unique_build_proof_declines_computed_keys_and_null_safe_equality() {
    ...
    let computed =
        crate::estimate::unique_keys::refresh_unique_keys(computed).expect("cache unique keys");
    ...
    let get = crate::estimate::unique_keys::refresh_unique_keys(declared_unique_get(&ctx))
        .expect("cache unique keys");
    condition.right =
        Expression::Reference(ReferenceExpression::new(1, LogicalType::BigInt).into());
