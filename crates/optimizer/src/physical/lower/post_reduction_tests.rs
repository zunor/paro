...
fn aggregate_lowers_post_reduction_into_separate_local_reference_domains() {
    ...
    let spec = lower_aggregate(&ctx, aggregate, PhysicalBuildContext::default(), false);
    assert_eq!(
...
fn lower_aggregate(
    ...
    let plan = PhysicalPlanBuilder::new(context)
        .build(root)
...
fn parallel_context() -> PhysicalBuildContext {
