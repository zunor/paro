...
use crate::statement::{ExplainEnvelope, QueryStatementLayer};
use paro_common::error::{self as paro_error, Result};
use paro_context::compile_diagnostics::Observation::Observed;
use paro_context::compile_diagnostics::SearchStop;
use paro_context::CompileReceiptSummary;
...
use crate::region::aggregate as aggregate_region;
mod statement;
#[cfg(test)]
mod tests;
...
impl Optimizer {
    pub(super) fn optimize_pipeline(
        ...
        plan = crate::rewrite::normalize::predicates(plan, &mut Default::default());
        ...
        fn select_access(
            ...
            if let Some(provider) = crate::physical::access::optimizer::SearchOptimizer::new()
        ...
        let mut selection = loop {
            ...
            if let Some(selected) = select::select(
                &candidate.plan,
                &self.ctx.column_stats,
                grant,
                &self.calibration,
