...
fn production_deferral_preserves_partial_group_facts_across_transport() {
    ...
    let mut plan = candidate(false);
    let LogicalOperator::Aggregate(root) = &mut plan.operator else {
        unreachable!()
    };
    let LogicalOperator::Join(Join::Comparison(join)) = &mut root.child.operator else {
        unreachable!()
    };
    let LogicalOperator::ExpressionGet(fact) = &mut join.left.operator else {
        unreachable!()
    };
    fact.expressions = (0..40)
        .map(|row| {
            vec![
                Expression::Constant(
                    ConstantExpression::new(Value::Integer(row % 4), LogicalType::Integer).into(),
                ),
                Expression::Constant(
                    ConstantExpression::new(Value::Double(row as f64), LogicalType::Double).into(),
                ),
            ]
        })
        .collect();
    ...
    let statistics = Arc::new(HashMap::from([
        ...
        (
            ...
            Arc::new(ColumnStatistics::with_estimated_distinct(
                paro_storage::statistics::BaseStatistics::create_unknown(LogicalType::Double),
                Some(40),
            )),
        ),
    ]));
    ...
    let PatternOperand::Expression { children, .. } = &mut binding.root else {
        unreachable!()
    };
    let PatternOperand::Expression { children, .. } = &mut children[0] else {
        unreachable!()
    };
    let fact_group = match children[0] {
    ...
    let (expected, expected_column) = {
        ...
        let (rewritten, changed) =
            dimension_deferral::optimize_plan(instantiated.plan, &state.bind_context).unwrap();
        assert!(changed);
        ...
        let settled = settlement::SettlementCache::default()
            .settle_arena_test_in(rewritten, &environment, &mut arena)
            .unwrap()
            .unwrap();
        ...
        for index in arena.post_order(settled.plan).unwrap() {
            ...
            if let LogicalOperator::Aggregate(aggregate) = &node.operator {
                if aggregate.groups.len() == 1
                    && aggregate.groups[0].equals(&col(0, 0, LogicalType::Integer))
                {
                    assert_eq!(
                        aggregate.group_stats[0]
                            .as_ref()
                            .unwrap()
                            .get_distinct_count(),
                        4
                    ...
                    partial = Some((
                        node.stats.clone(),
                        settled.scopes[&node.id][&ColumnBinding::new(aggregate.group_index, 0)]
                            .clone(),
                    ));
                }
            }
    ...
    while let Some((group, payload_id, children)) = pending.pop() {
        ...
        if let LogicalOperator::Aggregate(aggregate) = &payload.semantic_template.operator {
            ...
            {
                checked += 1;
                if aggregate.group_stats[0]
                    .as_ref()
                    .map(|s| s.get_distinct_count() as u64)
                    != Some(expected_column.distinct_evidence().point)
                {
                    failures.push(format!(
                        "partial group_stats did not retain known input NDV: {:?}",
                        aggregate.group_stats[0]
                    ));
                }
                let output = payload
                    .column_stats
                    .get(&ColumnBinding::new(aggregate.group_index, 0));
                if output.map(|s| s.distinct_evidence())
                    != Some(expected_column.distinct_evidence())
                {
                    failures
                        .push("new partial namespace has no matching output column fact".into());
                }
                if context.memo().cardinality_estimate(group).unwrap().1
                    != expected.estimated_cardinality.unwrap().expected
                {
                    failures.push(format!(
                        "partial rows {:?}, owned {:?}",
                        context.memo().cardinality_estimate(group),
                        expected
                    ));
                }
            }
    ...
    assert!(
        failures.is_empty(),
        "transport-dependent partial aggregate facts: {failures:?}"
    );
}
...
fn production_deferral_inlines_nonidentity_projection_spines() {
    for (depth, nary) in [(1, false), (2, false), (0, true), (2, true)] {
        ...
        assert_eq!(
            rule.planner_state.read().unwrap().staging_arena.len(),
            arena
        );
    }
}
