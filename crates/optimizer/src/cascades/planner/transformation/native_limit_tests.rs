...
fn plan(topn: bool, count: i32, offset: i32) -> OwnedLogicalPlan {
    ...
    for topn in [false, true] {
        for (count, offset) in [(-1, 0), (0, 0), (3, 1), (8192, 0), (3, -1)] {
            ...
            assert_eq!(
                outputs.len(),
                expected,
                "topn={topn}, count={count}, offset={offset}"
            );
            ...
            if let Some(output) = outputs.first() {
                ...
                if topn {
                    ...
                    let LogicalOperator::Limit(limit) = &state.payloads.logical[payload.index()]
                        .semantic_template
                        .operator
                    else {
