...
#[test]
fn lazy_grant_probe_retains_verified_per_class_baselines() {
    let mut budget = super::super::budget::SearchBudget::default();
    ...
    registry
        ...
        .unwrap();
    let mut engine = CascadesEngine::new(memo, registry);
    engine.lazy_grant_probe_enabled = true;
    let optimized = engine
        .optimize_for_grants(
            root,
            goal,
        ...
        .unwrap();
    assert_eq!(optimized.winners.len(), 2);
    assert!(engine.optional_grant_probe.is_none());
    for winner in &optimized.winners {
        assert_eq!(winner.goal.grant, GrantGoalKey::Class(winner.class));
        assert_eq!(winner.frozen.winner.candidate, winner.winner.candidate);
        assert!(winner.winner.cost.peak_memory_upper <= 1 << 30);
        assert!(winner.frozen.children.iter().all(|child|
            child.reference.goal.grant == GrantGoalKey::Invariant(AdmissibleGrantSetId(9))));
    }
}
