...
use crate::estimate::join::RelationsSetToStats;

#[derive(Debug, Clone)]
pub(crate) struct EqualityGraphVertex {
    pub(crate) relation: usize,
    pub(crate) binding: ColumnBinding,
}

#[derive(Debug, Clone, Copy)]
pub(crate) struct EqualityGraphEdge {
...
#[derive(Debug, Clone)]
pub(crate) struct EqualityClassGraph {
    pub(crate) stats_index: usize,
    pub(crate) vertices: Vec<EqualityGraphVertex>,
    pub(crate) edges: Vec<EqualityGraphEdge>,
...
impl EqualityClassGraph {
    pub(crate) fn build_all(stats_sets: &[RelationsSetToStats]) -> Vec<Self> {
        stats_sets
            .iter()
            .enumerate()
...
pub(crate) fn find_component(parents: &mut [usize], index: usize) -> usize {
    let mut root = index;
