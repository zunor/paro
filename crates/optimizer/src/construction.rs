...
pub(crate) fn predicates(
    ...
    scalars: &mut crate::expression::CanonicalScalars,
) -> OwnedLogicalPlan {
    let mut plan = crate::filter::pushdown::FilterPushdown::new().rewrite_plan(plan);
    if scalars.normalize_plan(&mut plan) {
        plan = crate::filter::pushdown::FilterPushdown::new().rewrite_plan(plan);
    }
    plan
}
    ...
    fn predicate_construction_matches_separate_routing_oracle() {
        ...
        for barrier in [false, true] {
            for nesting in 0..12 {
                ...
                crate::expression::scalar_normalizer().rewrite_plan(&mut expected);
                let mut expected = FilterPushdown::new().rewrite_plan(expected);
                let mut actual =
                    predicates(plan, &mut crate::expression::CanonicalScalars::default());
