...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let path = std::env::args().nth(1).ok_or("usage: validate_compile_record FILE")?;
    ...
    println!("schema={} invocation={} outcome={:?} (schema validation only)", record.schema_version, record.invocation, record.outcome);
    Ok(())
}
