...
use crate::physical::specs::ExpressionScanSpec;
use crate::runtime::context::{OperatorCallContext, PipelineInitContext};
use crate::runtime::source::SourcePoll;
use crate::runtime::state::{
    ExpressionSourceGlobal, ExpressionSourceLocal, SourceGlobal, SourceLocal,
};

#[derive(Debug, Clone)]
pub struct ExpressionSourceExec {
    pub spec: ExpressionScanSpec,
}

impl ExpressionSourceExec {
    pub(crate) fn create_global(&self, _ctx: &mut PipelineInitContext) -> Result<SourceGlobal> {
        Ok(SourceGlobal::Expression(Arc::new(ExpressionSourceGlobal {
    ...
    pub(crate) fn poll_next(
        ...
        let SourceLocal::Expression(local) = local else {
