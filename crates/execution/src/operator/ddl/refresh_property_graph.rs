...
use super::property_graph_support::{
    build_graph_index_from_scans, graph_data_dir, graph_statistics_from_scans, scan_graph_inputs,
...
use paro_planner::binder::ir::statement::BoundRefreshPropertyGraphInfo;
...
impl PhysicalOperator for RefreshPropertyGraph {
    fn operator_type(&self) -> PhysicalOperatorType {
        PhysicalOperatorType::RefreshPropertyGraph
    }
    ...
    fn get_data(
        ...
        let graph_entry = schema.get_property_graph(&txn, &self.info.graph_name)?;
        let graph_info = graph_entry.info.clone();
        let schema_fingerprint = graph_schema_fingerprint(&graph_info);
        let scanned = scan_graph_inputs(ctx, &graph_info)?;
        let graph_id = GraphId::new(
