...
impl JoinKeyLayout {
    ...
    /// Hash a serialized build row with the same physical kernels used for
    /// vector input. Exact integer joins defer this work while their compact
    /// index is still viable; if the index later declines, canonical hashes
    /// are reconstructed without boxing key values.
    pub(crate) fn hash_build_row(&self, build_layout: &BuildRowLayout, row_ptr: *const u8) -> u64 {
        let mut hash = HASH_SEED;
        for (col_idx, column_kind) in self.columns.iter().copied().enumerate() {
            let value_hash = if row_value_is_null(build_layout.base(), row_ptr, col_idx) {
                NULL_HASH
            } else {
                match column_kind {
                    JoinKeyColumn::Boolean => hash_u64(u64::from(read_row_bool(
                        build_layout.base(),
                        row_ptr,
                        col_idx,
                    ))),
                    JoinKeyColumn::I8 => {
                        hash_i64(read_row_i8(build_layout.base(), row_ptr, col_idx) as i64)
                    }
                    JoinKeyColumn::I16 => {
                        hash_i64(read_row_i16(build_layout.base(), row_ptr, col_idx) as i64)
                    }
                    JoinKeyColumn::I32 => {
                        hash_i64(read_row_i32(build_layout.base(), row_ptr, col_idx) as i64)
                    }
                    JoinKeyColumn::I64 => {
                        hash_i64(read_row_i64(build_layout.base(), row_ptr, col_idx))
                    }
                    JoinKeyColumn::I128 => {
                        hash_u128(read_row_i128(build_layout.base(), row_ptr, col_idx) as u128)
                    }
                    JoinKeyColumn::U8 => {
                        hash_u64(read_row_u8(build_layout.base(), row_ptr, col_idx) as u64)
                    }
                    JoinKeyColumn::U16 => {
                        hash_u64(read_row_u16(build_layout.base(), row_ptr, col_idx) as u64)
                    }
                    JoinKeyColumn::U32 => {
                        hash_u64(read_row_u32(build_layout.base(), row_ptr, col_idx) as u64)
                    }
                    JoinKeyColumn::U64 => {
                        hash_u64(read_row_u64(build_layout.base(), row_ptr, col_idx))
                    }
                    JoinKeyColumn::U128 => {
                        hash_u128(read_row_u128(build_layout.base(), row_ptr, col_idx))
                    }
                    JoinKeyColumn::F32 => hash_u64(u64::from(read_row_u32(
                        build_layout.base(),
                        row_ptr,
                        col_idx,
                    ))),
                    ...
                    JoinKeyColumn::Varchar | JoinKeyColumn::Blob => {
                        hash_bytes(read_row_varlen_bytes(build_layout.base(), row_ptr, col_idx))
                    }
                    JoinKeyColumn::ValueFallback { type_hash } => combine_hash(
                        type_hash,
                        value_fallback_hash(&build_layout.read_value(row_ptr, col_idx)),
                    ),
                }
            };
