...
impl Display for CTE {
    ...
    fn fmt(&self, f: &mut Formatter) -> std::fmt::Result {
        ...
        match self.materialization {
            CTEHint::Default => {}
            CTEHint::Materialized => write!(f, "MATERIALIZED ")?,
            CTEHint::NotMaterialized => write!(f, "NOT MATERIALIZED ")?,
        }
        write!(f, "({})", self.query)?;
        Ok(())
    }
        ...
        match self {
            ...
            TimeTravelPoint::Stream {
                ...
                write_dot_separated_list(
                    f,
                    database.iter().chain(schema.iter()).chain(Some(name)),
                )?;
                write!(f, ")")?;
            }
        }
...
pub struct TableAlias {
    ...
    /// When true, keep the original schema name on bound columns even after aliasing.
    pub keep_schema_name: bool,
}
