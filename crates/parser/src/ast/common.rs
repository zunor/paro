...
impl ColumnID {
    ...
    pub fn span(&self) -> crate::Span {
        match self {
            ColumnID::Name(id) => id.span,
            ColumnID::Position(id) => id.span,
        }
    }
}
...
impl Display for TableRef {
    fn fmt(&self, f: &mut Formatter) -> std::fmt::Result {
        assert!(self.database.is_none() || (self.database.is_some() && self.schema.is_some()));
        if let Some(database) = &self.database {
            write!(f, "{}.", database)?;
        }
...
impl ColumnRef {
    pub fn is_qualified(&self) -> bool {
        self.schema.is_some() || self.table.is_some()
    }
}

impl Display for ColumnRef {
    fn fmt(&self, f: &mut Formatter) -> std::fmt::Result {
        assert!(self.schema.is_none() || (self.schema.is_some() && self.table.is_some()));
