...
impl Literal {
    pub fn as_double(&self) -> Result<f64> {
        match self {
            ...
            _ => Err(ParseError::without_span(format!(
...
pub struct FunctionCall {
    ...
    pub distinct: bool,
    /// Schema of the function (e.g., `pg_catalog` in `pg_catalog.pg_get_userbyid()`)
    pub schema: Option<Identifier>,
    pub name: Identifier,
    pub args: Vec<Expr>,
    pub params: Vec<Expr>,
    pub order_by: Vec<OrderByExpr>,
    /// Aggregate filter clause, e.g. `SUM(x) FILTER (WHERE y > 0)`.
...
pub enum BinaryOperator {
    ...
    RegexpMatch,
    RegexpNotMatch,
    RegexpMatchInsensitive,
    RegexpNotMatchInsensitive,
    SoundsLike,
    BitwiseOr,
    BitwiseAnd,
    BitwiseXor,
...
impl BinaryOperator {
    ...
    pub fn to_func_name(&self) -> String {
        match self {
            ...
            BinaryOperator::RegexpNotMatchInsensitive => "not_regexp_insensitive".to_string(),
            _ => {
...
impl Display for BinaryOperator {
    fn fmt(&self, f: &mut Formatter) -> std::fmt::Result {
        match self {
            ...
            BinaryOperator::NegInnerProduct => {
                write!(f, "<#>")
            }
        }
    }
