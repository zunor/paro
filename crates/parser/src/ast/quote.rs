...
use crate::parser::default_ident_quote;
...
//
// There are also two known legacy parser special cases which do not require quoting:
...
pub fn display_ident(
    ...
    if name.chars().any(|c| c.is_ascii_uppercase()) && quoted_ident_case_sensitive
        ...
        || force_quoted_ident
    {
        QuotedIdent(name, default_ident_quote()).to_string()
    } else {
        name.to_string()
    }
}
