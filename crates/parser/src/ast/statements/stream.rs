...
pub struct CreateStreamStmt {
    ...
    pub table_schema: Option<Identifier>,
...
impl Display for CreateStreamStmt {
    fn fmt(&self, f: &mut Formatter) -> std::fmt::Result {
        ...
        write!(f, " ON TABLE ")?;
        write_dot_separated_list(f, self.table_schema.iter().chain(Some(&self.table)))?;
...
impl Display for DescribeStreamStmt {
    fn fmt(&self, f: &mut Formatter) -> std::fmt::Result {
        ...
        write_dot_separated_list(
            ...
            self.database
                .iter()
                .chain(self.schema.iter().chain(Some(&self.stream))),
        )
    }
}
