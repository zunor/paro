    ...
    fn fmt(&self, f: &mut Formatter) -> std::fmt::Result {
        match self {
            ...
            CreateTableSource::Like {
                ...
                write!(f, "LIKE ")?;
                write_dot_separated_list(f, database.iter().chain(schema).chain(Some(table)))
            }
        }
    }
}
...
impl Display for DescribeTableStmt {
    fn fmt(&self, f: &mut Formatter) -> std::fmt::Result {
        ...
        write_dot_separated_list(
            ...
            self.database
                .iter()
                .chain(self.schema.iter().chain(Some(&self.table))),
        )
    }
}
...
impl Display for VacuumDropTableStmt {
    fn fmt(&self, f: &mut Formatter) -> std::fmt::Result {
        ...
        if self.database.is_some() || self.schema.is_some() {
            write!(f, "FROM ")?;
            write_dot_separated_list(f, self.database.iter().chain(&self.schema))?;
            write!(f, " ")?;
        }
        write!(f, "{}", &self.option)?;
