...
use crate::parser::graph_pattern::graph_table_ref;
use crate::parser::input::Input;
use crate::parser::input::WithSpan;
use crate::parser::shared::hint::hint;
use crate::parser::shared::hint::top_n;
use crate::parser::shared::stage::file_location;
use crate::parser::shared::stage::select_stage_option;
...
pub fn with(i: Input) -> IResult<With> {
    let cte_materialization = alt((
        value(CTEHint::NotMaterialized, rule! { NOT ~ MATERIALIZED }),
        value(CTEHint::Materialized, rule! { MATERIALIZED }),
    ));

    let cte = map(
        consumed(rule! {
            #table_alias_without_as ~ AS ~ #cte_materialization? ~ "(" ~ #query ~ ")"
        }),
        |(span, (table_alias, _, materialization, _, query, _))| CTE {
            span: transform_span(span.tokens),
            alias: table_alias,
            materialization: materialization.unwrap_or(CTEHint::Default),
            query: Box::new(query),
        },
    );
...
pub fn travel_point(i: Input) -> IResult<TimeTravelPoint> {
    let at_stream = map(
        ...
        |(_, _, _, (database, schema, name), _)| TimeTravelPoint::Stream {
            database,
            schema,
            name,
        },
...
pub enum TableReferenceElement {
    ...
    TableFunction {
        ...
        alias: Option<TableAlias>,
        /// Whether WITH ORDINALITY is specified (adds row number column)
...
pub fn table_reference_element(i: Input) -> IResult<WithSpan<TableReferenceElement>> {
    maybe_grow_parser_stack(|| {
        ...
        let table_function = map(
            rule! {
                LATERAL? ~ #function_name ~ "(" ~ #comma_separated_list0(table_function_param) ~ ")" ~ (WITH ~ ORDINALITY)? ~ #table_alias? ~ SAMPLE? ~ (BLOCK ~ "(" ~ #expr ~ ")")? ~ (ROW ~ "(" ~ #expr ~ ROWS? ~ ")")?
            },
            |(lateral, name, _, params, _, with_ordinality, alias, sample, level, sample_conf)| {
        ...
        let graph_table = map(
            rule! {
                #graph_table_ref ~ #table_alias?
            },
            |(graph_table, alias)| TableReferenceElement::GraphTable { graph_table, alias },
        );
    ...
    for TableReferenceParser
{
    ...
    fn primary(&mut self, input: Self::Input) -> Result<Self::Output, &'static str> {
        let table_ref = match input.elem {
            ...
            TableReferenceElement::GraphTable { graph_table, alias } => {
