...
const fn binary_affix(op: &BinaryOperator) -> Affix {
    match op {
        ...
        | BinaryOperator::RegexpNotMatch
        | BinaryOperator::RegexpMatchInsensitive
        | BinaryOperator::RegexpNotMatchInsensitive => {
        ...
        BinaryOperator::NegInnerProduct => Affix::Infix(Precedence(22), Associativity::Left),
...
impl<'a, I: Iterator<Item = WithSpan<'a, ExprElement>>> PrattParser<I> for ExprParser {
    ...
    fn postfix(
        ...
        let expr = match elem.elem {
            ...
            ExprElement::DotAccess { key } => {
                ...
                if let Expr::ColumnRef { column, .. } = &mut lhs {
                    if let ColumnID::Name(name) = &column.column {
                        column.schema = column.table.take();
            ...
            ExprElement::ChainFunctionCall { name, args, lambda } => {
                // Check if lhs is a simple ColumnRef (only column, no schema/table)
                // If so, treat it as the schema for the function call
                // e.g., pg_catalog.pg_get_userbyid(x) -> FunctionCall { schema: pg_catalog, name: pg_get_userbyid, args: [x] }
                let (schema, final_args) = match &lhs {
                    Expr::ColumnRef { column, .. }
                        if column.schema.is_none() && column.table.is_none() =>
                    {
                        // lhs is a simple identifier, use it as schema
                        let schema_name = match &column.column {
                            ColumnID::Name(ident) => Some(ident.clone()),
                            _ => None,
                        };
                        (schema_name, args)
                    }
                    _ => {
                        // lhs is a complex expression, use it as the first argument
                        // e.g., arr.map(x -> x + 1) -> map(arr, x -> x + 1)
                        (None, [vec![lhs], args].concat())
                    }
                };
                Expr::FunctionCall {
                    span: transform_span(elem.span.tokens),
...
pub fn expr_element(i: Input) -> IResult<WithSpan<ExprElement>> {
    ...
    let timestamp_tz_expr = map(
        rule! {
            ( TIMESTAMP_TZ | TIMESTAMPTZ ) ~ #consumed(literal_string)
        },
        |(_, (span, date))| ExprElement::Cast {
            expr: Box::new(Expr::Literal {
                span: transform_span(span.tokens),
    ...
    try_dispatch!(i, true,
        ...
        Plus
            ...
            | ExclamationMarkTildeAsterisk => with_span!(rule!{ #binary_op | #unary_op }).parse(i),
        RArrow
            | LongRArrow
            | HashRArrow
            | HashLongRArrow
        ...
        Factorial | SquareRoot | BitWiseNot | CubeRoot | Abs => with_span!(rule!{ #binary_op | #unary_op }).parse(i),
...
pub fn binary_op(i: Input) -> IResult<BinaryOperator> {
    if let Some(token_0) = i.tokens.first() {
        op_branch!(
            ...
            NEGINNERPRODUCT => BinaryOperator::NegInnerProduct,
            ...
            BitWiseNot => BinaryOperator::RegexpMatch,
            TildeAsterisk => BinaryOperator::RegexpMatchInsensitive,
            ExclamationMarkTilde => BinaryOperator::RegexpNotMatch,
            ExclamationMarkTildeAsterisk => BinaryOperator::RegexpNotMatchInsensitive,
        );
        match token_0.kind {
            LIKE => {
...
pub fn literal_string(i: Input) -> IResult<String> {
    map_res(
        ...
        |token| {
            let text = token.text();
            let is_escape = text.starts_with('E') || text.starts_with('e');
            let input = if is_escape { &text[1..] } else { text };

            let quote::QuotedString(s, quote) = input
                .parse()
                .map_err(|_| nom::Err::Failure(ErrorKind::Other("invalid escape or unicode")))?;

            if !crate::parser::input::is_string_quote(quote) {
...
pub fn type_name(i: Input) -> IResult<TypeName> {
    ...
    let ty_uint32 = value(
        TypeName::UInt32,
        rule! { (
                #map(rule! { UINT ~ ( "(" ~ ^#literal_u64 ~ ^")" )? }, |(t, _)| t) |
    ...
    let ty_uhugeint = value(
        TypeName::UHugeInt,
        rule! { UHUGEINT ~ ( "(" ~ ^#literal_u64 ~ ^")" )? },
    );
    let ty_int8 = value(
        TypeName::Int8,
    ...
    let ty_hugeint = value(
        TypeName::HugeInt,
        rule! { HUGEINT ~ ( "(" ~ ^#literal_u64 ~ ^")" )? },
    );
    ...
    let ty_list = map(
        rule! { LIST ~ "(" ~ #type_name ~ ")" },
        |(_, _, item_type, _)| TypeName::List(Box::new(item_type)),
    );
    let ty_map = map(
    ...
    let ty_tuple = map(
        rule! { ( TUPLE | STRUCT ) ~ "(" ~ #comma_separated_list1(type_name) ~ ")" },
    ...
    let ty_named_tuple = map_res(
        rule! { ( TUPLE | STRUCT ) ~ "(" ~ #comma_separated_list1(rule! { #ident ~ #type_name }) ~ ")" },
        |(_, _, fields, _)| {
    ...
    let ty_uuid = value(TypeName::Uuid, rule! { UUID });
    let ty_char = value(
        TypeName::Char,
        rule! { ( CHAR | CHARACTER ) ~ ( "(" ~ ^#literal_u64 ~ ^")" )? },
    );
    let ty_string = value(
        TypeName::String,
        rule! { ( STRING | VARCHAR | TEXT ) ~ ( "(" ~ ^#literal_u64 ~ ^")" )? },
    );
    let ty_tsvector = value(TypeName::TsVector, rule! { TSVECTOR });
    let ty_tsquery = value(TypeName::TsQuery, rule! { TSQUERY });
    let ty_json = value(TypeName::Json, rule! { JSON });
    let ty_jsonb = value(TypeName::Jsonb, rule! { JSONB });
    ...
    let ty_timestamp_tz_simply = value(TypeName::TimestampTz, rule! { TIMESTAMP_TZ | TIMESTAMPTZ });
    map_res(
        alt((
            rule! {
            ( #ty_boolean
            ...
            | #ty_map
            | #ty_bitmap
            | #ty_tuple : "TUPLE/STRUCT(<type>, ...)"
            | #ty_named_tuple : "TUPLE/STRUCT(<name> <type>, ...)"
            ) ~ #nullable? : "type name"
            },
            rule! {
            ( #ty_date
...
pub fn function_call(i: Input) -> IResult<ExprElement> {
    ...
    let function_call_body = map_res(
        ...
        |(
            ...
            match (
                ...
                (first_param, None, opt_distinct, params, None, Some(order_by), filter, window) => {
                ...
                (first_param, None, opt_distinct, params, None, None, filter, Some(window)) => {
                    let mut args = params.unwrap_or_default();
                    if let Some(first_param) = first_param {
                        args.insert(0, first_param)
                    }
                ...
                (first_param, None, opt_distinct, params, None, None, filter, None) => {
                    let mut args = params.unwrap_or_default();
                    if let Some(first_param) = first_param {
                        args.insert(0, first_param)
                    }
    ...
    map(
        rule!(
            #function_name
            ~ #function_call_body : "`function(... [ , x -> ... ] ) [ (...) ] [ WITHIN GROUP ( ORDER BY <expr>, ... ) ] [ FILTER ( WHERE <expr> ) ] [ OVER ([ PARTITION BY <expr>, ... ] [ ORDER BY <expr>, ... ] [ <window frame> ]) ]`"
        ),
