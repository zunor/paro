    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self.lexer.next() {
            Some(TokenKind::Error) => {
                let error_start = self.lexer.span().start;
                let span = Some((error_start..self.source.len()).into());

                // Try to provide a more helpful error message for common mistakes
                let error_char = self.source[error_start..].chars().next();
                let error_msg = match error_char {
                    Some('，') => {
                        "found fullwidth comma '，' (U+FF0C), use ASCII comma ',' instead"
                            .to_string()
                    }
                    Some('；') => {
                        "found fullwidth semicolon '；' (U+FF1B), use ASCII semicolon ';' instead"
                            .to_string()
                    }
                    Some('：') => {
                        "found fullwidth colon '：' (U+FF1A), use ASCII colon ':' instead"
                            .to_string()
                    }
                    Some('（') => {
                        "found fullwidth left parenthesis '（' (U+FF08), use ASCII '(' instead"
                            .to_string()
                    }
                    Some('）') => {
                        "found fullwidth right parenthesis '）' (U+FF09), use ASCII ')' instead"
                            .to_string()
                    }
                    Some('\u{2018}') | Some('\u{2019}') => {
                        "found curly quote, use straight single quote ' instead".to_string()
                    }
                    Some('\u{201C}') | Some('\u{201D}') => {
                        "found curly quote, use straight double quote \" instead".to_string()
                    }
                    Some(c) if !c.is_ascii() => {
                        format!("found non-ASCII character '{}' (U+{:04X})", c, c as u32)
                    }
                    _ => "unable to recognize the rest tokens".to_string(),
                };

                Some(Err(ParseError::new(span, error_msg)))
            }
            Some(kind) => {
...
pub enum TokenKind {
    ...
    DESC,
    #[token("DESTINATION", ignore(ascii_case))]
    DESTINATION,
    ...
    HOURS,
    #[token("HUGEINT", ignore(ascii_case))]
    HUGEINT,
    ...
    SHA256_PASSWORD,
    #[token("SHORTEST", ignore(ascii_case))]
    SHORTEST,
    ...
    TIMESTAMP_TZ,
    #[token("TIMESTAMPTZ", ignore(ascii_case))]
    TIMESTAMPTZ,
    ...
    TRY_CAST,
    #[token("TSQUERY", ignore(ascii_case))]
    TSQUERY,
    #[token("TSV", ignore(ascii_case))]
    TSV,
    #[token("TSVECTOR", ignore(ascii_case))]
    TSVECTOR,
    ...
    TYPE,
    #[token("UHUGEINT", ignore(ascii_case))]
    UHUGEINT,
