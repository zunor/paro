...
use crate::expression::{ComparisonType, Expression, OperatorType, ReferenceExpression};
...
pub struct AggregateSpec {
    ...
    /// Immutable optimizer decision. Runtime memory pressure may exercise an
    /// allowed spill path, but cannot upgrade a no-spill plan or infer a
    /// forced external algorithm from mutable session state.
...
pub fn verify_local_expression(
