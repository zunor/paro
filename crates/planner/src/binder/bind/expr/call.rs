    ...
    match resolve_scalar_callable(binder, schema, name, &arg_types)? {
        ResolvedScalarCallable::Native(entry) => {
            ...
            Ok(Expression::Function(
                FunctionExpression::new(bound_function, bound_args, return_type).into(),
            ))
        }
...
fn bind_aggregate_function(
    ...
    Ok(Expression::Aggregate(
        (bind_aggregate_invocation(binder, schema, name, args, distinct, filter, order_bys)?)
            .into(),
    ))
}
