use crate::binder::ir::BoundStatementKind;
use crate::binder::Binder;
use paro_common::error::Result;
use paro_parser::ast::RefreshPropertyGraphStmt;

#[derive(Debug, Clone)]
pub struct BoundRefreshPropertyGraphInfo {
    pub catalog_name: String,
    pub schema_name: String,
    pub graph_name: String,
}

pub fn bind_refresh_property_graph(
    binder: &mut Binder,
    stmt: RefreshPropertyGraphStmt,
) -> Result<BoundStatementKind> {
    let catalog_name = binder.catalog().name().to_string();
    let schema_name = binder.session_context().current_schema().to_string();
    let graph_name = stmt.graph_name.name;
    let txn = binder.catalog_txn_view();
    let schema = binder.catalog().get_schema(&txn, &schema_name)?;
    schema.get_property_graph(&txn, &graph_name)?;

    Ok(BoundStatementKind::RefreshPropertyGraph(
        BoundRefreshPropertyGraphInfo {
            catalog_name,
            schema_name,
            graph_name,
        },
    ))
}
