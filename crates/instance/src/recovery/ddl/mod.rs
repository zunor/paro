...
impl<'a> CatalogReplayHandler<'a> {
    pub(in crate::recovery) fn replay_catalog_ops_by_phase(
        ...
        ops: &[CatalogTxnOp],
        commit_id: u64,
        predicate: impl Fn(&CatalogTxnOp) -> bool,
    ) -> paro_common::error::Result<()> {
        for op in ops.iter().filter(|op| predicate(op)) {
            self.replay_catalog_txn_op(op, commit_id)?;
        }
        Ok(())
    }

    pub(in crate::recovery) fn replay_catalog_ops_in_commit_order(
        &mut self,
        ops: &[CatalogTxnOp],
        commit_id: u64,
    ) -> paro_common::error::Result<()> {
        self.replay_catalog_ops_by_phase(ops, commit_id, |op| {
            matches!(
                &op.change.change,
        ...
        self.replay_catalog_ops_by_phase(ops, commit_id, |op| {
            matches!(&op.change.change, DdlChange::AlterEntry(_))
        })?;
        Ok(())
    }
