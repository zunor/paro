...
fn tokenize_with_script_boundaries<F>(text: &str, out: &mut Vec<Token>, is_script_char: F)
where
    F: Fn(char) -> bool,
{
    let mut position: TokenPosition = 0;
    let mut alnum_buffer = String::new();

    for ch in text.chars() {
        if is_script_char(ch) {
            flush_alnum_token(&mut alnum_buffer, &mut position, out);
            out.push(Token::new(ch.to_string(), position));
            position += 1;
            continue;
        }

        if ch.is_alphanumeric() {
            alnum_buffer.push(ch);
        } else {
            flush_alnum_token(&mut alnum_buffer, &mut position, out);
        }
    }
...
fn tokenize_basic_words(text: &str, out: &mut Vec<Token>) {
    let mut position: TokenPosition = 0;
    for raw in text.split(|ch: char| !ch.is_alphanumeric()) {
        if raw.is_empty() {
            continue;
        }
        out.push(Token::new(raw.to_lowercase(), position));
        position += 1;
    }
}
...
fn retain_and_reposition<F>(out: &mut Vec<Token>, mut keep: F)
where
    F: FnMut(&str) -> bool,
{
...
impl Tokenizer for DefaultTokenizer {
    fn tokenize(&self, text: &str, out: &mut Vec<Token>) {
        ...
        for raw in text.split(|ch: char| !ch.is_alphanumeric()) {
            ...
            out.push(Token::new(term, position));
            position += 1;
        }
    }
}
...
impl Tokenizer for EnglishTokenizer {
    fn tokenize(&self, text: &str, out: &mut Vec<Token>) {
        tokenize_basic_words(text, out);
        retain_and_reposition(out, |term| !is_english_stopword(term));
        for token in out.iter_mut() {
            token.term = snowball_like_english_stem(token.term.as_str());
        }
        retain_and_reposition(out, |term| !term.is_empty());
    }
...
impl Tokenizer for ChineseTokenizer {
    fn tokenize(&self, text: &str, out: &mut Vec<Token>) {
        tokenize_with_script_boundaries(text, out, is_han);
        retain_and_reposition(out, |term| !is_chinese_stopword(term));
    }
...
impl Tokenizer for JapaneseTokenizer {
    fn tokenize(&self, text: &str, out: &mut Vec<Token>) {
        tokenize_with_script_boundaries(text, out, |ch| {
            is_han(ch) || is_hiragana(ch) || is_katakana(ch)
        });
        retain_and_reposition(out, |term| !is_japanese_stopword(term));
    }
