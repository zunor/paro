...
impl<T> BinaryOperator<T, T, T> for DivOperator
where
    T: Div<Output = T> + Copy,
{
    #[inline]
...
impl<T> BinaryOperator<T, T, T> for ModOperator
where
    T: Rem<Output = T> + Copy,
{
    #[inline]
...
pub fn register_arithmetic_functions(set: &mut ScalarFunctionSet) {
    ...
    match name.as_str() {
        ...
        "/" => {
            add_numeric_signatures::<DivOperator>(set, &name);
        }
        "%" => {
            add_numeric_signatures::<ModOperator>(set, &name);
        }
        _ => {}
    }
}
