...
impl UnaryExecutor {
    ...
    pub fn execute<INPUT, RESULT, OP>(input: &Vector, result: &mut Vector, count: usize)
    where
        INPUT: Copy + 'static,
        RESULT: Copy,
    ...
    fn execute_flat<INPUT, RESULT, OP>(input: &Vector, result: &mut Vector, count: usize)
    where
        INPUT: Copy + 'static,
        RESULT: Copy,
        OP: UnaryOperator<INPUT, RESULT>,
    ...
    fn execute_constant<INPUT, RESULT, OP>(input: &Vector, result: &mut Vector, _count: usize)
    where
        INPUT: Copy + 'static,
        RESULT: Copy,
        OP: UnaryOperator<INPUT, RESULT>,
    ...
    fn execute_view<INPUT, RESULT, OP>(input: &Vector, result: &mut Vector, count: usize)
    where
        INPUT: Copy + 'static,
        RESULT: Copy,
        OP: UnaryOperator<INPUT, RESULT>,
...
mod tests {
    ...
    fn test_execute_dictionary_input() {
        let child = Vector::from_i64(&[-5, 0, 7]);
        let dict = Vector::dictionary(Arc::new(child), vec![2, 0, 1]);
        let mut result = Vector::with_capacity(LogicalType::BigInt, 3);
    ...
    fn test_execute_sequence_input() {
        let input = Vector::sequence(-3, 2, 4);
    ...
    fn test_execute_dictionary_over_sequence_respects_nulls() {
        let mut sequence = Vector::sequence(-4, 3, 4);
        sequence.validity_mut().set_null(1);
        let dict = Vector::dictionary(Arc::new(sequence), vec![3_u32, 0, 1]);
        let mut result = Vector::with_capacity(LogicalType::BigInt, 3);
