...
benchmark/.venv/bin/python benchmark/harness/cold_planning_gate.py \
  --report benchmark/report/cold-q11.json \
  --baseline benchmark/report/cold-q11-baseline.json
```

The gate checks both median client EXPLAIN wall time and internal optimizer
component time, plus sampled peak process RSS. It rejects absent/failed blocks,
...
typed diagnostic rows, binary/source/harness/data/SQL hashes and settings are
retained. RSS sampling has a 20 ms target interval (plus `ps` overhead), so it is
